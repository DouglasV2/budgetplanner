...
/**
 * Sprint 10.41 — Portugal (PT), the 11th market: IKEA-only (no JYSK in Portugal), like FR/ES/IT. Ported from
 * the IKEA Italy set via the article-number trick to {@code /pt/pt/}; each row's Portuguese name (og:title) +
 * per-market EUR price (JSON-LD) + verified og:image read off ikea.com/pt on 2026-06-18. Proves the file
 * imports cleanly, every row is market=PT / IKEA / planner-eligible with a verified image, the core rooms are
 * covered, and the planner builds a real (non-partial) PT plan.
 */
class PortugalCatalogRuntimeTest {

    @Test
    void portugalCatalogImportsCleanlyWithVerifiedImagesAcrossRooms() throws Exception {
        List<RetailerProductSnapshotDto> snapshot = load("/catalog/real-ikea-pt-rooms.json");
        assertThat(snapshot).as("IKEA PT rows").hasSizeGreaterThanOrEqualTo(55);
        ...
        assertThat(saved).allSatisfy(product -> {
            ...
            assertThat(product.getProductUrl()).startsWith("https://www.ikea.com/pt/");
        ...
        for (String room : List.of("living-room", "bedroom", "home-office", "dining-room")) {
            assertThat(saved).as("PT catalog covers %s", room)
                    .anySatisfy(p -> assertThat(p.getRoomTags()).contains(room));
        }
    }

    @Test
    void plannerBuildsANonPartialPortugueseLivingRoom() throws Exception {
        List<Product> catalog = importAll(load("/catalog/real-ikea-pt-rooms.json"));
        ...
        PlanGenerationResponse plan = planner.generate(new PlannerInputDto(
                "Sala até 1500 €, moderna, IKEA.", 1500, "living-room",
                "modern", "Lisboa", 24, "multi", List.of("IKEA"), "best-value", "comfort",
                List.of(), List.of(), List.of(), List.of(), List.of(), 0).withMarket("PT"));
