...
class RetailerCollectorServiceTest {
    ...
    @Test
    void collectsValidProductsAndKeepsRunningWhenOneUrlFails() throws Exception {
        ...
        CollectorRunSummaryDto summary = service.collect(
                new CollectorRequestDto("JYSK", List.of(okUrl, noPriceUrl, brokenUrl), defaults));
        ...
        assertThat(summary.importSummary().created()).isEqualTo(1);

        ArgumentCaptor<Product> captor = ArgumentCaptor.forClass(Product.class);
        verify(repository).save(captor.capture());
        assertThat(captor.getValue().getSourceType()).isEqualTo("future-scraper");
        assertThat(captor.getValue().getRetailer()).isEqualTo("JYSK");
    }
    ...
    void rejectsTooManyUrls() {
        ...
        CollectorRunSummaryDto summary = service.collect(new CollectorRequestDto("IKEA", urls, defaults));
    ...
    void rejectsUnsupportedRetailer() {
        ...
        CollectorRunSummaryDto summary = service.collect(
                new CollectorRequestDto("Nepoznata", List.of("https://example.com/p/x"), defaults));
    ...
    private RetailerCollectorService service(ProductPageFetcher fetcher, ProductRepository repository) {
        ...
        return new RetailerCollectorService(fetcher, new RetailerProductParser(new ObjectMapper()), snapshotImportService);
    }
